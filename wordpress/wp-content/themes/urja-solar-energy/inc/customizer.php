...
function urja_solar_energy_customize_register( $wp_customize ) {
	...
	$wp_customize->add_setting('urja_solar_energy_google_plus_url',array(
		'default'	=> '',
		'sanitize_callback'	=> 'esc_url_raw'
	));	
	$wp_customize->add_control('urja_solar_energy_google_plus_url',array(
		'label'	=> __('Add Google Plus link','urja-solar-energy'),
		'section'	=> 'urja_solar_energy_topbar_header',
		'setting'	=> 'urja_solar_energy_google_plus_url',
		'type'	=> 'url'
	));
		...
		$manager->add_section(
			new Urja_Solar_Energy_Customize_Section_Pro(
				...
				array(
					...
					'pro_url'  => esc_url( 'https://www.luzuk.com/themes/solar-energy-wordpress-theme/' ),
				)
			)
		);
	}
